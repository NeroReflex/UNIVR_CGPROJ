...
const char *vertexShaderSource =
    ...
    "layout (location = 1) in vec3 in_aNorm;\n"
    ...
    "layout (location = 1) out vec3 out_aNorm;\n"
    ...
    "   out_aNorm = in_aNorm * normal_matrix;\n"
    "}\0";
...
const char *geometryShaderSource =
    ...
    "layout (location = 2) out vec3 out_aNorm;\n"
    "layout (location = 3) out flat vec3 out_aTangent;\n"
    "\n"
    "void main()\n"
    "{\n"
    "   vec3 edge1_modelspace = in_aPos[1] - in_aPos[0];\n"
    "   vec3 edge2_modelspace = in_aPos[2] - in_aPos[0];\n"
    "   vec2 deltaUV1 = in_aTexCoord[1] - in_aTexCoord[0];\n"
    "   vec2 deltaUV2 = in_aTexCoord[2] - in_aTexCoord[0];\n"
    "   \n"
    ...
    "       \n"
    "       out_aPos = in_aPos[i];\n"
    "       out_aTexCoord = in_aTexCoord[i];\n"
    "       out_aNorm = normal_matrix * in_aNorm[i];\n"
    "       \n"
    "       out_aTangent = normalize( /* normal_matrix * */ tangent );\n"
    "       \n"
    "       EmitVertex();\n"
    "   }\n"
    "   EndPrimitive();\n"
...
const char *fragmentShaderSource =
    ...
    "layout (location = 2) in vec3 in_aNorm;\n"
    "layout (location = 3) flat in vec3 in_aTangent;\n"
    "\n"
    "layout(location = 0) out vec4 color;\n"
    "\n"
    "uniform vec3 lightColor;\n"
    ...
    "void main()\n"
    "{\n"
    "   vec3 bitangent = cross(in_aNorm, in_aTangent);\n"
    "   mat3 TBN = mat3(in_aTangent, bitangent, in_aNorm);\n"
    "   mat3 TBN_inv = transpose(inverse(TBN));\n"
    "\n"
    "   vec3 diffuseTerm = texture(material.diffuseTex, in_aTexCoord).rgb;\n"
    "\n"
    "   vec3 lightDir = normalize(lightPos - in_aPos);\n"
    "   vec3 lightDir_tangentspace = TBN_inv * lightDir;\n"
    "\n"
    "   vec3 ambientContrib = vec3(ambient);\n"
    "\n"
    "   vec3 normal_tangentspace = texture(material.normalMapTex, in_aTexCoord).rgb;\n"
    "   vec3 diffuseContrib = lightColor * max(dot(normalize(normal_tangentspace), lightDir_tangentspace), 0.0);\n"
    "\n"
    "   color = vec4(ambientContrib * diffuseTerm + diffuseContrib * diffuseTerm, 1.0);\n"
    "}\0";
...
int main()
{
    ...
    auto geometry = std::unique_ptr<GeometryShader>(GeometryShader::CompileShader(geometryShaderSource));
    ...
    auto shaderProgram = std::unique_ptr<Program>(Program::LinkProgram(vertex.get(), geometry.get(), fragment.get()));
    ...
    unsigned char *diffuse_texture_data = stbi_load("images/brickwall.jpg", &diffuse_width, &diffuse_height, &diffuse_nrChannels, 0);
    assert(diffuse_texture_data != nullptr);
    assert(diffuse_nrChannels == 3);

    int specular_width, specular_height, specular_nrChannels;
    unsigned char *specular_texture_data = stbi_load("images/brickwall_normal.jpg", &specular_width, &specular_height, &specular_nrChannels, 0);
    assert(specular_texture_data != nullptr);
    assert(specular_nrChannels == 3);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        shaderProgram->texture("material.normalMapTex", 1, *specularTextureObj.get());
