...
static float up_pressed = true;
static float down_pressed = true;
static void process_input(GLFWwindow *const window)
{
    ...
    if (glfwGetKey(window, GLFW_KEY_UP) == GLFW_PRESS) {
        if (!up_pressed)
            mixValue = glm::min(mixValue + 0.1f, 1.0f);
    ...
    if (glfwGetKey(window, GLFW_KEY_DOWN) == GLFW_PRESS) {
        if (!down_pressed)
            mixValue = glm::max(mixValue - 0.1f, 0.0f);
...
int main()
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        shaderProgram->bind();
        shaderProgram->uniformFloat("mixValue", mixValue);
