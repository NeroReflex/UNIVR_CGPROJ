...
const char *fragmentShaderSource =
    ...
    "void main()\n"
    "{\n"
    "   vec3 normal_viewspace = normalize(view * vec4(in_aNormal, 0.0)).xyz;\n"
    "   vec3 position_viewspace = normalize(view * vec4(in_aPos, 1.0)).xyz;\n"
    // la posizione dell'osservatore in viewspace e' sempre (0,0,0):
    // vedere esercizio 36
    "   vec3 viewPos_viewspace = normalize(view * vec4(viewPos, 1.0)).xyz;\n"
    ...
    "\n"
    "   vec3 diffuseContrib = lightColor * max(dot(normalize(normal_viewspace), lightDir_viewspace), 0.0);\n"
    "\n"
    ...
    "   vec3 specularContrib = lightColor * specularStrength * pow(max(dot(viewDir_viewspace, reflectDir_viewspace), 0.0), shininess);\n"
    "\n"
