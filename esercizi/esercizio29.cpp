...
// Rispetto agli esercizi precedenti la z è inversa perchè glm::lookAt inverte
// la direzione della vista
...
static glm::mat4 build_view_matrix(void) {
    ...
    return glm::lookAt(camera_center, camera_center + glm::normalize(camera_dir), camera_up);
}
...
static void process_input(GLFWwindow *const window, glm::float32 deltaTime)
{
    ...
    const auto no_y_camera_dir = glm::normalize(glm::vec3(get_camera_direction().x, 0.0f, get_camera_direction().z));

    if (glfwGetKey(window, GLFW_KEY_ESCAPE) == GLFW_PRESS)
        glfwSetWindowShouldClose(window, true);
    if (glfwGetKey(window, GLFW_KEY_W) == GLFW_PRESS)
        camera_center += delta_mov * no_y_camera_dir;
...
std::optional<glm::float32> lastX;
std::optional<glm::float32> lastY;
