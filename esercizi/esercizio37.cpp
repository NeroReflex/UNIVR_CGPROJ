...
// assolutamente orribile da vedere: la luce speculare non e' corretta:
...
const char *vertexShaderSource =
    ...
    "\n"
    "layout (location = 0) out vec3 ambientContrib;\n"
    "layout (location = 1) out vec3 diffuseContrib;\n"
    "layout (location = 2) out vec3 specularContrib;\n"
    "\n"
    "void main()\n"
    "{\n"
    ...
    "   vec3 out_aPos = worldspace_pos.xyz;\n"
    "   vec3 out_aNormal = normal_matrix * in_aNormal;\n"
    "   vec3 normal_viewspace = normalize(view * vec4(out_aNormal, 0.0)).xyz;\n"
    "   vec3 position_viewspace = normalize(view * vec4(out_aPos, 1.0)).xyz;\n"
    "   vec3 viewPos_viewspace = vec3(0.0, 0.0, 0.0);\n"
    "   vec3 lightPos_viewspace = normalize(view * vec4(lightPos, 1.0)).xyz;\n"
    "\n"
    "   vec3 lightDir_viewspace = normalize(lightPos_viewspace - position_viewspace);\n"
    "\n"
    "   ambientContrib = vec3(ambient);\n"
    "\n"
    "   diffuseContrib = lightColor * max(dot(normalize(normal_viewspace), lightDir_viewspace), 0.0);\n"
    "\n"
    "   vec3 reflectDir_viewspace = reflect(-lightDir_viewspace, normal_viewspace);\n"
    "   vec3 viewDir_viewspace = normalize(viewPos_viewspace - position_viewspace);\n"
    "   specularContrib = lightColor * specularStrength * pow(max(dot(viewDir_viewspace, reflectDir_viewspace), 0.0), shininess);\n"
    "\n"
    "}\0";
...
const char *fragmentShaderSource =
    ...
    "precision highp float;\n"
    "\n"
    "layout (location = 0) in vec3 ambientContrib;\n"
    "layout (location = 1) in vec3 diffuseContrib;\n"
    "layout (location = 2) in vec3 specularContrib;\n"
    "\n"
...
const char *vertexShaderSourceLightsource =
    ...
    "{\n"
    "   mat4 transform = projection * view * model;\n"
    "   gl_Position = transform * vec4(in_aPos, 1.0);\n"
    "}\0";
...
int main()
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        shaderProgram->uniformFloat("shininess", 128.0f);
