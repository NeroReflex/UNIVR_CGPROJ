...
const char *fragmentShaderSource =
    ...
    "   vec3 emissiveTerm = texture(material.emissiveTex, in_aTexCoord).rgb;\n"
    "\n"
    ...
    "\n"
    "   vec3 emissionContrib = vec3(material.emissionStrength);\n"
    "\n"
    ...
    "   color = vec4(\n"
    "       (\n"
    "           ambientContrib * diffuseTerm +\n"
    "           diffuseContrib * diffuseTerm +\n"
    "           specularContrib * specularTerm +\n"
    "           emissionContrib * emissiveTerm\n"
    "       ),\n"
    "       1.0\n"
    "    );\n"
    "}\0";
...
int main()
{
    ...
    auto emissiveTextureObj = std::shared_ptr<Texture>();
    ...
    int emissive_width, emissive_height, emissive_nrChannels;
    unsigned char *emissive_texture_data = stbi_load("images/matrix.jpg", &emissive_width, &emissive_height, &emissive_nrChannels, 0);
    assert(emissive_texture_data != nullptr);
    assert(emissive_nrChannels == 3);

    {
        ...
        emissiveTextureObj = std::shared_ptr<Texture>(
            Texture::Create2DTexture(
                emissive_width,
                emissive_height,
    ...
    assert(emissiveTextureObj != nullptr);
    assert(diffuseTextureObj != nullptr);
    assert(specularTextureObj != nullptr);

    stbi_image_free(emissive_texture_data);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        shaderProgram->texture("material.emissiveTex", 1, *emissiveTextureObj.get());
        shaderProgram->uniformFloat("material.shininess", 32.0f);
        shaderProgram->uniformFloat("material.emissionStrength", 0.94f);
